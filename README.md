...
- **출처가 있는 FAQ 답변 (RAG)**: 환불·수하물 같은 정책 질문은 FAQ 33건(`backend/app/data/faq.json`)을 pgvector 유사도 검색으로 찾아 **근거로만** 답하고 출처를 표기합니다. 거리 임계값을 넘는 근거가 없으면 지어내지 않고 "확인이 어려워요"라고 안내하며, 임베딩 API가 없으면 키워드 검색으로 전환합니다. 구조 상세: [`docs/faq-rag.md`](docs/faq-rag.md) · 시연: [`demo/trip-agent-demo-faq.mp4`](demo/trip-agent-demo-faq.mp4) (25초)
