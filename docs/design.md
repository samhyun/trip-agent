...
핵심 설계 원칙: **툴은 데이터 출처를 모른다.** 서비스 레이어가 도메인·목적지에 따라
provider를 고르고, 실패하면 mock으로 폴백한다. (조사·비교 상세는 [`data-sources.md`](data-sources.md))
...
services (도메인 로직 + provider 선택)
    ...
    ▼
providers                              data/
  ├─ tour_api.py    (국내 관광·숙박)     ├─ destinations.json
  ├─ opentripmap.py (전세계 명소 POI)    ├─ hotels.json
  ├─ duffel.py      (해외 항공)          ├─ flights.json
  ├─ liteapi.py     (해외 호텔)          └─ activities.json
  └─ weather.py     (날씨, 선택)
```
...
| 도메인 | provider | 폴백 |
|---|---|---|
| 명소/볼거리 (국내+해외) | OpenTripMap | mock |
| 국내 관광·숙박·축제 | TourAPI | mock |
| 해외 항공 | Duffel | mock |
| 해외 호텔 | LiteAPI | mock |
| 결제 | 더미(mock) | — |
...
│   │   ├── services/              # 여행 도메인 로직 + provider 선택
│   │   ├── providers/             # tour_api, opentripmap, duffel, liteapi, weather
│   │   └── data/                  # mock JSON
│   └── tests/
