...
| `destination` | 명소 조회 + **대표 사진(국내 TourAPI·해외 Wikipedia)**. 큐레이션 밖 도시는 **자동 해석** · 취향(관심사) 필터 |
...
| 해외 명소 (POI) | **Geoapify** (지오코딩 + LLM 명소생성) | ✅ **연동** | mock |
...
> 사진=그라디언트로 대체된다(단, 에이전트 구동엔 LLM 키 필요).
