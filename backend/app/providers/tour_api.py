...
def search_attractions(city: str, limit: int = 8) -> list[dict] | None:
    ...
    # 큐레이션 키워드마다 searchKeyword2 1회. 순차 호출 시 (키워드 수 × 최대 6s)로 20~30s까지 걸려
    # 독립 호출을 병렬 조회한다(→ 최대 1건 지연 수준으로 단축). 결과는 키워드 순서 유지.
    # 전체 키워드를 조회한 뒤 앞에서부터 limit개를 채운다(일부 검색 실패 시 뒤 키워드로 백필).
    # areaCode·contentTypeId 미지정(둘 다 걸면 정확한 명소명이 0건 나오는 TourAPI 특성).
    keywords = FAMOUS_SPOTS.get(city, [])
    result: list[dict] = []
    seen: set = set()  # contentid(없으면 이름) 기준 중복 제거

    def _add(it: dict) -> None:
        key = it.get("contentid") or (it.get("title") or "").strip()
    ...
    if len(result) < limit:  # 큐레이션 없음/일부 실패 → 지역기반 목록(제목순)으로 부족분 백필
        items = _call("areaBasedList2", areaCode=area_code, contentTypeId=12, arrange="O", numOfRows=limit + 5)
        for it in items or []:
            if len(result) >= limit:
                break
            _add(it)
    ...
    # 비어있지 않으면 캐시(플레이키한 API에서 반복 6~12s 재조회 방지). 부족분은 백필로 최소화.
